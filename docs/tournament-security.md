...
- `TournamentCore.csv` добавляет BOM для Excel, экранирует разделитель/кавычки/переводы строк и префиксует формульные значения (`=`, `+`, `@`), чтобы имя игрока не стало Excel-формулой. Для UI также доступен HTML-табличный `.xls` экспорт без формул.
