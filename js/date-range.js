// js/date-range.js — общий фильтр по датам для списков раундов
// (docs/CODE-REVIEW.md п.3 — дробление utils.js по ответственности).
// Вынесено из js/utils.js целиком: блок зависит только от фундамента —
// t()/currentLang (i18n), fmtDate() (format) и escapeHtml() (dom), и все эти
// вызовы происходят во время рендера, а не на загрузке. Владельцы состояния —
// только этот файл (dateRangeFilters), наружу отдаётся API initDateRangeFilter.
// Грузится как часть ESM-бандла (src/date-range.js → dist/livescoring-modules.js).
