...
function buildPlayerSlots() {
    ...
    for (var i = 1; i <= count; i++) {
        ...
        Object.entries(registeredUsers).forEach(function(e) {
            var u = e[1];
            var sel = (i === 1 && e[0] === currentUser.uid) ? 'selected' : '';
...
function fillPlayerFromUser(idx) {
    var uid = document.getElementById('pl-select-' + idx).value;
    if (!uid) return;
    var u = registeredUsers[uid];
    if (!u) return;
    document.getElementById('pl-name-' + idx).value = u.name || '';
    document.getElementById('pl-hcp-' + idx).value = u.handicap != null ? u.handicap : '';
    if (u.gender) document.getElementById('pl-gender-' + idx).value = u.gender;
    calcPlayerFieldHcp(idx);
}

function calcPlayerFieldHcp(idx) {
    var hcp = document.getElementById('pl-hcp-' + idx).value;
    var gender = document.getElementById('pl-gender-' + idx).value;
    var tee = document.getElementById('g-tee').value;
    if (!hcp) { document.getElementById('pl-field-' + idx).value = ''; return; }
    var field = getFieldHcp(parseFloat(hcp), tee, gender);
    document.getElementById('pl-field-' + idx).value = field;
}
...
function startGroup() {
    ...
    for (var i = 1; i <= count; i++) {
        var uid = document.getElementById('pl-select-' + i).value;
        var name = document.getElementById('pl-name-' + i).value.trim();
        ...
        players[pid] = {
            name: name, exactHcp: hcp ? parseFloat(hcp) : null, fieldHcp: fieldHcp,
            gender: gender, scores: {}, markerScores: {}, verified: {}
        };
        playerIdsOrder.push(pid);
    }

    // АВТОМАТИЧЕСКОЕ ЦИКЛИЧЕСКОЕ НАЗНАЧЕНИЕ МАРКЕРОВ
    var markerAssignments = {};
    for (var i = 0; i < playerIdsOrder.length; i++) {
        var markerId = playerIdsOrder[i];
        var targetId = playerIdsOrder[(i + 1) % playerIdsOrder.length];
        players[targetId].markedBy = markerId;
        markerAssignments[markerId] = { targetId: targetId, targetName: players[targetId].name };
    }
    ...
    var data = {
        mode: 'group', tee: tee, format: format,
        startHole: startHole, startTime: startDate.getTime(),
        players: players, markerAssignments: markerAssignments,
        status: 'active', createdAt: Date.now(), createdBy: currentUser.uid,
        participants: playerIdsOrder
    };
...
function initRoundView() {
    db.ref('rounds/' + curRid).on('value', function(sn) {
        curRoundData = sn.val();
        if (!curRoundData) { toast('Раунд не найден', 'error'); return; }

        document.getElementById('mode-view').classList.add('hidden');
        var pageSub = document.getElementById('page-sub');
        if (pageSub) pageSub.textContent = curRoundData.format + ' · ТИ: ' + TEES[curRoundData.tee];

        var isParticipant = myUid && curRoundData.players && curRoundData.players[myUid];
...
function findCurrentHole() {
    ...
    var myScores = curRoundData.players[myUid].scores || {};
    playHole = order[0];
    for (var i = 0; i < order.length; i++) {
        if (!(parseInt(myScores[order[i]]) > 0)) { playHole = order[i]; break; }
    }
}
...
function buildPlayHolesNav() {
    ...
    var order = holeOrder(curRoundData.startHole || 1);
    var myScores = curRoundData.players[myUid].scores || {};
    var myVerified = curRoundData.players[myUid].verified || {};
    var html = '';
...
function goPlayHole(h) {
    isChanging = true;
    playHole = h; myScore = 0; targetScore = 0;
...
function renderPlayHole() {
    ...
    var myScoresObj = curRoundData.players[myUid].scores || {};
    var mySaved = parseInt(myScoresObj[playHole]) || 0;
    myScore = mySaved > 0 ? mySaved : par;

    if (myTargetUid) {
        var markerScoresObj = curRoundData.players[myTargetUid].markerScores || {};
        var myMarkerData = markerScoresObj[myUid] || {};
        var targetSaved = parseInt(myMarkerData[playHole]) || 0;
...
function checkPlayVerification() {
    ...
    var markerScoresObj = curRoundData.players[myUid].markerScores || {};
    var markerData = myMarkerId ? (markerScoresObj[myMarkerId] || {}) : {};

    var myS = parseInt(myScoresObj[playHole]) || 0;
    var markerS = parseInt(markerData[playHole]) || 0;

    if (myS > 0 && markerS > 0 && myS === markerS) {
        box.innerHTML = '<div class="verify-ok">✅ Ваш счёт подтверждён маркером</div>';
    } else if (myS > 0 && markerS > 0 && myS !== markerS) {
        box.innerHTML = '<div class="verify-fail">⚠️ НЕСОВПАДЕНИЕ! Вы: ' + myS + ' | Маркер: ' + markerS + '</div>';
    } else if (myS > 0) {
        box.innerHTML = '<div class="verify-wait">⏳ Ждём подтверждение от маркера</div>';
    } else {
        box.innerHTML = '';
    }
}
...
function saveHoleScores() {
    ...
    if (myMarkerId) {
        var markerScoresObj = curRoundData.players[myUid].markerScores || {};
        var myMarkerScore = parseInt((markerScoresObj[myMarkerId] || {})[h]) || 0;
    ...
    if (myTargetUid) {
        var targetScoresObj = curRoundData.players[myTargetUid].scores || {};
        var targetSelfScore = parseInt(targetScoresObj[h]) || 0;
...
function renderPlaySummary() {
    ...
    Object.entries(curRoundData.players).forEach(function(pe) {
        ...
        html += '<div style="font-size:12px;color:var(--muted);">Лунок: ' + stats.holesPlayed + '</div></div>';
...
function callOfficial(type) {
    var typeName = type === 'referee' ? 'Судья' : 'Маршал';
    if (!confirm('Вызвать ' + typeName.toLowerCase() + 'а на лунку ' + playHole + '?')) return;
    db.ref('alerts').push({
        roundId: curRid, type: type, hole: playHole,
        playerId: myUid, playerName: curRoundData.players[myUid].name,
        time: Date.now(), status: 'active'
    }).then(function() {
        toast('🚨 ' + typeName + ' вызван на лунку ' + playHole + '!', 'warn');
    });
}
