...
(function (root) {
    ...
    function protocolPanelHtml(t) {
        ...
        return '<div class="tna-panel"><h3><i class="fas fa-file-signature"></i> ' + tr('Финальный протокол', 'Final protocol') + '</h3><p class="tna-panel-sub">' + tr('Снимок результатов по лункам создаётся из существующих rounds + settings/course. Версия неизменяема после публикации; экспорт CSV открывается в Excel.', 'The per-hole result snapshot uses existing rounds + settings/course. Published versions are immutable; CSV opens in Excel.') + '</p><div class="tna-panel-sub">' + tr('Номинации фиксируются вместе с версией:', 'Nominations are stored with the version:') + '</div><div class="tna-mini-actions" style="margin-bottom:11px;">' + protocolNominationOptions(t) + '</div><div class="tna-toolbar"><span class="tna-status ' + (state.state === 'published' ? 'completed' : state.state === 'fixed' ? 'closed' : 'active') + '">' + esc(state.state) + '</span><span class="tn-public-chip">v' + state.version + '</span><div class="tna-actions">' + controls + actionButton('protocol', t._key, tr('Открыть протокол', 'Open protocol'), 'fa-file-pdf', 'btn-og') + (t.protocol && t.protocol.rows && t.protocol.rows.length ? actionButton('protocol-export', t._key, 'CSV / Excel', 'fa-file-csv', 'btn-og') : '') + '</div></div></div>';
    }
