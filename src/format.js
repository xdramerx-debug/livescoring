// ESM canonical for formatting helpers (score + date/time).
// Mirrors js/format.js. Uses the global t()/currentLang (i18n) only at call
// time, so it is safe to load before js/utils.js. Exposes globals on window
// during the migration (see src/course-config.js).
...
if (typeof window !== 'undefined') {
    Object.assign(window, {
        ...
        fmtDate, fmtTime, tnDateTs, normalizeTimestampMs, isTodayTimestamp
    });
}
...
export function tnDateTs(dateStr) {
...
export function isTodayTimestamp(ts, nowTs) {
