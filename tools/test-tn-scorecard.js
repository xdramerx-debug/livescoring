...
['1', '2', '3', '4', '5'].forEach(function(v) {
    ...
    hasnt('нет SI', html, '>SI<');
    hasnt('нет Stbl', html, 'Stbl');
    hasnt('нет строки форы', html, '>Фора<');
    hasnt('нет отдельной ячейки форы', html, 'club-sc-hcp');
    has('фора внутри счёта', html, 'club-sc-handicap');
...
hasnt('нет Stableford в общей карточке', sampleHtml, 'Stableford');
...
check('группа: одна общая карточка на всех', (groupHtml.match(/<section class="club-sc/g) || []).length, 1);
has('группа: это общая карточка', groupHtml, 'club-sc-group');
check('группа: столбец каждого игрока', (groupHtml.match(/class="gsg-player"/g) || []).length, 2);
has('группа: красные ти', groupHtml, 'tee-rd');
has('группа: синие ти', groupHtml, 'tee-bl');
has('группа: имя экранировано', groupHtml, '&lt;img');
hasnt('группа: нет HTML из имени', groupHtml, '<img');
var soloGroupHtml = G.generateGroupHoleTableHTML({ players: { a: sample }, holeRange: '1-9' });
has('соло: личная карточка', soloGroupHtml, 'class="club-sc"');
hasnt('соло: не общая карточка', soloGroupHtml, 'club-sc-group');
var g1 = { name: 'Анна', tee: 'rd', fieldHcp: 10, scores: { 1: 4, 2: 5 } };
var g2 = { name: 'Борис', tee: 'bl', fieldHcp: 20, scores: { 1: 3, 2: 6 } };
var strokesHtml = G.generateGroupHoleTableHTML({ players: { a: g1, b: g2 }, holeRange: '1-9' });
has('группа: имя первого игрока', strokesHtml, 'Анна');
has('группа: имя второго игрока', strokesHtml, 'Борис');
has('группа: удары первого', strokesHtml, '>4<');
has('группа: удары второго', strokesHtml, '>3<');
has('группа: итоговая строка', strokesHtml, 'gsg-total');
var markerGroupHtml = G.generateGroupHoleTableHTML({ players: { a: Object.assign({}, g1, { markedBy: 'b', markerScores: { b: { 1: 5 } } }), b: g2 }, holeRange: '1-9' }, { showMarker: true });
has('группа: счёт маркера', markerGroupHtml, 'gsg-marker');
has('группа: расхождение подсвечено', markerGroupHtml, 'cell-mismatch');
has('группа: точка вместо префикса М', markerGroupHtml, 'gsg-mk-dot');
hasnt('группа: нет префикса «М:»', markerGroupHtml, 'М:');
hasnt('группа: нет префикса «M:»', markerGroupHtml, 'M:');
has('группа: легенда за иконкой i', markerGroupHtml, 'gsg-legend');
has('группа: легенда свёрнута', markerGroupHtml, '<details');
var longNameHtml = G.generateGroupHoleTableHTML({ players: { a: { name: 'Владимир Воробьев', tee: 'wh', fieldHcp: 10, scores: { 1: 4 } }, b: g2 }, holeRange: '1-9' });
has('группа: имя не обрезано', longNameHtml, 'Владимир');
has('группа: фамилия не обрезана', longNameHtml, 'Воробьев');
hasnt('группа: нет многоточия в имени', longNameHtml, 'Влад...');
has('группа: имя переносится по строкам', longNameHtml, 'gsg-pname-line');
var foraCap = G.hcpCaptionHTML(18, 1);
has('фора подписана полностью', foraCap, 'Фора +1');
hasnt('нет сокращения Ф+', foraCap, 'Ф+');
hasnt('фора не названа штрафом', foraCap, 'Штраф');
